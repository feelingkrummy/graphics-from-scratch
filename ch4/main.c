...
Vec3 reflect_ray(Vec3 r, Vec3 norm) {
	return vec3_sub(vec3_scale(norm, 2.0*vec3_dot(norm, r)), r);
}
...
ClosestIntersection closest_intersection(Vec3 origin, Vec3 direction, double t_min, double t_max) {
	...
	for (int i = 0; i < spheres_len; i += 1) {
		...
		if ( t.t1 > t_min && t.t1 < t_max && t.t1 < closest.t ) {
			closest.t = t.t1;
			closest.s = spheres[i];
			closest.found = true;
		}
		if ( t.t2 > t_min && t.t2 < t_max && t.t2 < closest.t ) {
			closest.t = t.t2;
			closest.s = spheres[i];
			closest.found = true;
		}
...
double compute_lighting(Vec3 point, Vec3 norm, Vec3 view, double specular_exp) {
	...
	for (int i = 0; i < lights_len; i += 1) {
		...
		if (light->type == AMBIENT) {
			...
			ClosestIntersection shadow = closest_intersection(point, light_ray, 0.0001, t_max);
			if (shadow.found) {
				continue;
			}
			...
			// Specular
			if (specular_exp != -1.0) {
				Vec3 R = reflect_ray(light_ray, norm);
...
Color trace_ray(Vec3 origin, Vec3 direction, double t_min, double t_max, int recursion_depth) {
	ClosestIntersection closest = closest_intersection(origin, direction, t_min, t_max);
	if (!closest.found) {
		return BLACK;
	}

	Vec3 point = vec3_add(origin, vec3_scale(direction, closest.t));
	Vec3 norm = vec3_sub(point, closest.s.center);
	norm = vec3_scale(norm, 1.0/vec3_len(norm));

	Vec3 view = vec3_scale(direction, -1);
	Color local_color = (Color){
		.r = clamp(closest.s.color.r * compute_lighting(point, norm, view, closest.s.specular), 0, 255),
		.g = clamp(closest.s.color.g * compute_lighting(point, norm, view, closest.s.specular), 0, 255),
		.b = clamp(closest.s.color.b * compute_lighting(point, norm, view, closest.s.specular), 0, 255)
	};

	if (recursion_depth <= 0 || closest.s.reflective <= 0) {
		return local_color;
	}

	Vec3 reflected = reflect_ray(view, norm);
	Color reflected_color = trace_ray(point, reflected, 0.001, INFINITY, recursion_depth-1);
	
	return (Color) {
		.r = local_color.r * (1 - closest.s.reflective) + reflected_color.r * closest.s.reflective,
		.g = local_color.g * (1 - closest.s.reflective) + reflected_color.g * closest.s.reflective,
		.b = local_color.b * (1 - closest.s.reflective) + reflected_color.b * closest.s.reflective
	};
}
